...
def copy_server_content(addon_output_dir, current_dir, log):
    ...
    log.info(f"Copying server content:\n{current_dir} -> {addon_output_dir}")
...
def main():
    ...
    release = int(os.environ['REZ_BUILD_INSTALL'])

    output_dir = os.path.join(os.environ["REZ_BUILD_SOURCE_PATH"], "build")

    if release:
        output_dir = os.environ["REZ_BUILD_INSTALL_PATH"]

    log.info(f"Building into: {output_dir}")

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    else:
        log.warning(f"Wiping contents of {output_dir}")
        shutil.rmtree(output_dir)

    current_dir = os.path.dirname(os.path.abspath(__file__))
    copy_server_content(output_dir, current_dir, log)
    zip_client_side(output_dir, current_dir, log)
